...
if (!empty($actif) && '1' == $actif) {
    ...
    $plugin_output_new = $this->services->get(Utils::class)->strIreplacement(
...
if (
    ...
    && !empty($dblclic) && '1' == $dblclic && $this->HasAccess('write', $incPageName)
) {
...
if (('PageMenuHaut' == $incPageName || strstr($class, 'topnavpage')) && !strstr($class, 'horizontal-dropdown-menu')) {
    ...
    if (!is_null($dropdownslist)) {
        foreach ($dropdownslist as $element) {
            ...
            foreach ($nodes as $node) {
                ...
                } elseif ('#text' == $node->nodeName && '' == !trim($node->nodeValue)) {
...
$plugin_output_new = (!empty($clear) && '1' == $clear)
    ? $plugin_output_new.'<div class="clearfix"></div>'."\n"
    : $plugin_output_new;

$plugin_output_new = $this->services->get(Utils::class)->postFormat($plugin_output_new);
