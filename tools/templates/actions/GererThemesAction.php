...
class GererThemesAction extends YesWikiAction
{
    ...
    protected function modifyTheme()
    {
        ...
        if (!is_array($post->all('selectpage'))) {
        ...
        foreach ($pagesTags as $pageTag) {
            if ('reinitialiser' === $post->get('typemaj')) {
                ...
                if (!empty($presets) && ('.css' !== substr($presets, -4))) {
                    $presets .= '.css';
                }
                $this->pageManager->setMetadata($pageTag, [
                    'theme' => $theme,
                    'style' => $style.('.css' === substr($style, -4) ? '' : '.css'),
                    'squelette' => $squelette.('.tpl.html' === substr($squelette, -strlen('.tpl.html')) ? '' : '.tpl.html'),
                ] + (
