...
class FormManager
{
    ...
    public function getFromRawData($form)
    {
        ...
        [$template_list, $modify] = $this->prepare_with_special_parameters($form);
    ...
    public function create($data)
    {
        ...
        $query = 'INSERT INTO '.$this->dbService->prefixTable('nature')
                    ...
                    .($this->isAvailableOnlyOneEntryOption() ? ((isset($data['bn_only_one_entry']) && 'Y' === $data['bn_only_one_entry']) ? 'Y' : 'N').'", "' : '", "')
    ...
    public function update($data)
    {
        ...
        if ($activitypubEnabled && null === $data['bn_activitypub_private_key']) {
            $keyPair = $this->httpSignatureService->generateKeyPair();
            $privateKey = $keyPair[0];
            $publicKey = $keyPair[1];
        }
        ...
        return $this->dbService->query('UPDATE'.$this->dbService->prefixTable('nature').'SET '
            ...
            .($this->isAvailableOnlyOneEntryOption() ? '`bn_only_one_entry`="'.((isset($data['bn_only_one_entry']) && 'Y' === $data['bn_only_one_entry']) ? 'Y' : 'N').'",' : '')
    ...
    public function findNewId()
    {
        ...
        return max($vMaxCachedFormId, $vMaxDBFormIdHigherThan10000) + 1;
    }
    ...
    public function parseTemplate($raw)
    {
        ...
        foreach ($chaine as $ligne) {
            ...
            if (!empty($ligne) && !(false !== strrpos($ligne, '#', -strlen($ligne)))) {
    ...
    public function isAvailableOnlyOneEntryOption(): bool
    {
        if (is_null($this->isAvailableOnlyOneEntryOption)) {
            ...
            $this->isAvailableOnlyOneEntryOption = (0 !== @mysqli_num_rows($result));
        }
    ...
    public function isAvailableOnlyOneEntryMessage(): bool
    {
        if (is_null($this->isAvailableOnlyOneEntryMessage)) {
            ...
            $this->isAvailableOnlyOneEntryMessage = (0 !== @mysqli_num_rows($result));
        }
    ...
    protected function convertWithSpecialParameters($template, $id_nature)
    {
        ...
        $modify = false;
        for ($temp_index = 0; $temp_index < count($template_list); ++$temp_index) {
            if ('image' == $template_list[$temp_index][0]) {
                $modify = true;
