    ...
    public function run()
    {
        ...
        if (!empty($maxWidth) && is_numeric($maxWidth)) {
            ...
            if (empty($maxHeight) || !is_numeric($maxHeight)) {
                $maxHeight = ('4par3' == $this->arguments['ratio']) ? ($maxWidth * 3 / 4) : ($maxWidth * 9 / 16);
            } else {
                // calculte the minimum between width and height
                $newMaxHeight = min(('4par3' == $this->arguments['ratio']) ? ($maxWidth * 3 / 4) : ($maxWidth * 9 / 16), $maxHeight);
                $newMaxWidth = min(('4par3' == $this->arguments['ratio']) ? ($maxHeight * 4 / 3) : ($maxHeight * 16 / 9), $maxWidth);
            ...
            $manageSize = true;
            if (empty($maxWidth) || !is_numeric($maxWidth)) {
                $maxWidth = ('4par3' == $this->arguments['ratio']) ? ($maxHeight * 4 / 3) : ($maxHeight * 16 / 9);
            }
        }
